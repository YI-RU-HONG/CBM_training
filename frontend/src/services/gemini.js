...
- Most frequent emotion: ${data.userStats.emotionCounts ? Object.keys(data.userStats.emotionCounts)[0] || 'N/A' : 'N/A'}
` : ''}
...
export async function getMoodeeMessageGemini(data) {
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      console.log('🕐 Public function timed out, trying direct API');
    } else {
    ...
    try {
      ...
      if (fallbackError.name === 'AbortError') {
        console.log('🕐 All API methods timed out, using local fallback');
      } else {
