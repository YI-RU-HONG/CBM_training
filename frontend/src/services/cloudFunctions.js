...
export const publicGeminiMessage = async (data) => {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // create timeout controller - reduce timeout to 20 seconds for faster fallback
      const controller = new AbortController();
      const timeoutId = setTimeout(() => {
        console.log('🤖 Request timeout, aborting...');
        controller.abort();
      }, 20000); // 20 seconds timeout (reduced for faster fallback)
      ...
      // if it is AbortError, log as info instead of error to reduce noise
      if (error.name === 'AbortError') {
        console.log(`🕐 Attempt ${attempt} timed out (this is normal for slow API)`);
        if (attempt < maxRetries) {
          const delay = 3000; // fixed 3 seconds delay for timeout errors
  ...
  // Log final failure quietly if it's just timeout
  if (lastError && lastError.name === 'AbortError') {
    console.log('🕐 All requests timed out, using fallback response');
  } else {
