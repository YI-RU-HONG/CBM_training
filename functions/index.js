...
// temporarily comment out Firestore trigger functions to avoid permission issues
/*
// Cloud Function: when game is completed, automatically update statistics
exports.onGameCompleted = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    try {
      ...
      if (!username) {
        console.log('No username in game data, skipping statistics update');
        return;
      }
      ...
      console.log(`📊 Game completed for user ${username} (${uid}), updating statistics...`);
    ...
    res.json({ message: cleanMessage });
      ...
      if (fetchError.name === 'AbortError') {
        ...
        res.status(408).json({ 
          ...
          message: 'Sorry, I couldn\'t come up with something helpful this time.'
        });
      } else {
        throw fetchError;
      }
