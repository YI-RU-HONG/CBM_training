# CBM App 2 - Project Documentation
...
#### 6. Gemini API Trial Limitations
**Symptoms**: Long response times, timeouts, or rate limiting
**Root Cause**: Gemini API trial version has limitations on:
...
- **Progressive Enhancement**: Show local content first, enhance with AI
...
6. **Monitor API Limits**: Check Gemini API usage dashboard
...
3. **Local Fallbacks**: Immediate responses when API is slow
4. **Caching Strategy**: 5-minute cache to reduce API calls
5. **Background Processing**: AI calls don't block user interface
...
firebase deploy --only functions:publicGeminiMessage

# View function logs
firebase functions:log --only publicGeminiMessage
...
│   ├── Home/         # Homepage and main screens
